...
{
    if (friendUpdatePackage.type != UPDATE_ACCOUNT_INFO)
        return;
    ...
    for (int row = 0; row < model->rowCount(); ++row)
    {
        ...
        {
            // 更新所有显示相关数据
            QString avatarPath = user_Info.avatar_Path.isEmpty() ? ":/resource/image/avatar1.jpg" : user_Info.avatar_Path;
...
{
    if (onlineStatusPackage.type != UPDATE_ONLINE_STATUS)
        return;
...
{
    if (friendInfoSyncPackage.type != ASYNC_FETCH_FRIEND_LIST_INFOS)
        return;
...
{
    if (resend_Pkg.type != HANDLE_FRIEND_REQUEST)
        return;
